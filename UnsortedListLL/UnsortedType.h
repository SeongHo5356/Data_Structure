...
public:
  ...
  void MergeLists_B(UnsortedType<ItemType>& other, UnsortedType<ItemType>& reuslt);
...
template<class ItemType>
void MergeLists_A(UnsortedType<ItemType>& l_a, UnsortedType<ItemType>& l_b, UnsortedType<ItemType>& result) {

    
    
    ItemType item;
    // l_a 리스트의 아이템들을 result 에 삽입
    l_a.ResetList();
...
template<class ItemType>
void UnsortedType<ItemType>::MergeLists_B(UnsortedType<ItemType>& other, UnsortedType<ItemType>& reuslt) {
    ...
    location->next = other.listData;
    
    reuslt.listData = listData;

    /*
    NodeType<ItemType>* currentPos = listData;
    for (int i = 0; i < length; i++)
    {
        result.InsertItem(currentPos->info);
        currentPos = currentPos->next;
    }
    other.currentPos = other.listData;
    for (int i = 0; i < other.length; i++)
    {
        result.InsertItem(other.currentPos->info);
...
void UnsortedType<ItemType>::DeleteItem_B(ItemType item) {
    ...
    
    {
        ...
        while (location->next != NULL) 
        {
            ...
            {
                NodeType<ItemType>* tempLoctaion = location;
                location = location->next;
                length--;
                delete tempLoctaion;
            }
    ...
    /*
    {
        ...
        while (location->next != NULL) {
            ...
            if (location->info == item)
            {
                NodeType<ItemType>* tempLocation;
                tempLocation = location;
                location = location->next;		// Delete first node.
                length--;
                delete tempLocation;
            }
