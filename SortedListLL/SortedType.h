...
template<class ItemType>
void MergeLists_A(SortedType<ItemType>& l_a, SortedType<ItemType>& l_b, SortedType<ItemType>& result) {
    // l_a, l_b 리시트의 아이템들은 ResetList()와 GetNextItem()로 구성된 iterator를 사용하여 획득
    ItemType item;
...
// member function
template<class ItemType>
void SortedType<ItemType>::MergeLists_B(SortedType<ItemType>& other, SortedType<ItemType>& reuslt) {
    //1. NodeType의 포인터 변수 2개를 추가하고, 각각 listData와 other list의 listData를 가리키도록 한다.
    // ex) NodeType<ItemType> *ptr = listData;
    //2. 아이템을 비교 하며 합병을 시작한다. 단. 둘 리스트의 길이가 같다는 보장이 없으므로,
